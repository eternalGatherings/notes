...
document.addEventListener("DOMContentLoaded", () => initializeDynamicContent());
  ...
  // Hard-coded client-side credentials (change these values as needed)
  const HARDCODED_ADMIN_USER = 'sachinknight';
  const HARDCODED_ADMIN_PASS = 'evmFORhTiICgvAOBW123';
      ...
      // If already logged in, load admin content immediately; otherwise show login
      const loadAdminContent = async () => {
        ...
        try {
          ...
          root.innerHTML = html;
          // initialize handlers for injected content
      ...
      if (isLogged) {
        // load content without showing login
        loadAdminContent();
      } else {
        showLogin();
      }
      ...
      if (form) {
        form.addEventListener('submit', async (e) => {
          ...
          // Hard-coded credential check (client-side only)
          if (!u || !p) {
            if (msg) {
          ...
          if (u === HARDCODED_ADMIN_USER && p === HARDCODED_ADMIN_PASS) {
